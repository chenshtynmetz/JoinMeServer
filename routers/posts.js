...
postRouter.post('/addGroup', async (req, res) => {
  const userList = await post_functions.addGroupToDb(req.body.title, req.body.city, req.body.address, req.body.time, req.body.date, req.body.head_uid, req.body.min, req.body.max, req.body.wid)
  if (userList.length === 0 ) {
    return res.status(404).send()
  }
  return res.status(200).send("done")
