...
export async function getGroupDetails(gid) {
  ...
  if(groupDoc.exists()){
    ...
    const {title, city, address, time, date, num_of_participant, min_participants} = groupDoc.data()
    return {title, city, address, time, date, num_of_participant, min_participants, head_of_group_uid}
  }
  else{
    console.log("error")
    return null
